#!/usr/bin/env python
"""
基于 sys.settrace 的知识库构建管道逐行追踪 → 交互式 HTML 可视化。

原理：sys.settrace 是 Python 调试器（pdb）的底层机制，能在真实运行时
捕获每个函数的 call/line/return 事件，以及该行所有局部变量的快照。

用法: python scripts/trace_knowledge_base.py
输出: knowledge_base_trace.html（自包含，浏览器直接打开）
"""
...
NODES_FILE = str(PROJECT_ROOT / "scripts" / "build_knowledge_base.py")
NODES_NORM = os.path.normcase(os.path.normpath(NODES_FILE))

# ── 管道节点元数据（用于流程图展示）──────────────────────────────
PIPELINE_META = [
    {"key": "load_document",            "func": "load_document",
     "label": "① 文档加载",  "desc": "根据扩展名选择 Loader（PDF / Markdown）", "color": "#3b82f6"},
    {"key": "split_documents",          "func": "split_documents",
     "label": "② 统一分块入口", "desc": "根据文件类型自动选择分块策略", "color": "#10b981"},
    {"key": "split_pdf_documents",      "func": "split_pdf_documents",
     "label": "③ PDF 分块",  "desc": "过滤空页 + RecursiveCharacterTextSplitter", "color": "#f59e0b"},
    {"key": "split_markdown_documents", "func": "split_markdown_documents",
     "label": "④ Markdown 分块", "desc": "MarkdownHeaderTextSplitter + MarkdownTextSplitter 两阶段", "color": "#8b5cf6"},
]
FUNC_TO_NODE = {m["func"]: m["key"] for m in PIPELINE_META}
FUNC_TO_NODE["<module>"] = "__main__"

# ── 追踪存储 ──────────────────────────────────────────────────
_steps = []
_current_node = "__main__"
_step_counter = 0
_file_cache = {}
...
def _check_target(filename):
    ...
    if filename not in _file_cache:
        ...
        _file_cache[filename] = (fn == NODES_NORM)
...
def trace_fn(frame, event, arg):
    ...
    if event == "call":
        if not _check_target(frame.f_code.co_filename):
            return None
...
def run():
    ...
    if main_start is None:
        raise RuntimeError("在 build_knowledge_base.py 中找不到 if __name__ == '__main__' 块")

    import scripts.build_knowledge_base as kb_mod
    ...
    print("=" * 60)
    print("🚀 开始追踪知识库构建管道（真实运行）...")
    print("=" * 60)

    start = time.time()
    old_name = kb_mod.__dict__.get("__name__")
    kb_mod.__dict__["__name__"] = "__main__"
    try:
        exec(compile(padded, NODES_FILE, "exec"), kb_mod.__dict__)
    finally:
        if old_name is not None:
            kb_mod.__dict__["__name__"] = old_name
    ...
    print(f"\n📄 HTML 已生成，浏览器打开查看:")
    print(f"   {PROJECT_ROOT / 'knowledge_base_trace.html'}")
...
def generate_html(source_lines, elapsed):
    ...
    out = PROJECT_ROOT / "knowledge_base_trace.html"
...
<div class="toolbar">
  ...
  <button id="nextBtn" title="下一步 (j)">下一步</button>
  <button id="overBtn" title="下一行: 跳过函数调用 (l)">下一行</button>
  <button id="loopBtn" title="跳出循环 (n)">跳出循环</button>
  <button id="outBtn" title="跑完当前函数 (L)">跑完函数</button>
...
let _modalValues = [];
let _fullExpandVars = new Set();
let _modalVarName = null;
...
function renderPipeline(){
  ...
  if(current>=0){
    for(let i=0;i<=current;i++){
      ...
      if(n!=='__main__') doneNodes.add(n);
    }
    if(curNode && curNode!=='__main__') doneNodes.delete(curNode);
  }
  let h='';
  META.forEach((m,i)=>{
  ...
  const isMainActive = curNode==='__main__';
...
function jumpToMain(){
  const idx=STEPS.findIndex(s=>s.pipeline_node==='__main__');
  if(idx>=0){current=idx;render();}
}
...
function renderStep(){
  ...
  else if(s.pipeline_node==='__main__'){tag.textContent='· 主流程';tag.style.color='#6b7280';}
...
function renderVal(val,depth,maxExpandDepth,path){
  if(depth===undefined)depth=0;
  if(maxExpandDepth===undefined)maxExpandDepth=0;
  ...
  if(Array.isArray(val)){
    ...
    const items=val.map(function(v,i){return '<div class="indent">'+renderVal(v,depth+1,maxExpandDepth,path+'['+i+']')+'</div>';}).join('');
  ...
  if(typeof val==='object'){
    ...
    const usr=_userExpandState[path];
    const expanded=usr!==undefined?usr:def;
    const items=keys.map(function(k){return '<div class="indent"><span class="key">'+esc(k)+':</span> '+renderVal(val[k],depth+1,maxExpandDepth,path?path+'.'+k:k)+'</div>';}).join('');
    const cls=expanded?'':'hidden';
    const arrow=expanded?'▾':'▸';
...
function tog(path){
  ...
  if(isTopLevel){
    ...
    if(wasExpanded){
      ...
      Object.keys(_userExpandState).forEach(function(p){
...
function copyModal(){
  ...
  navigator.clipboard.writeText(text).then(function(){const b=document.querySelector('.modal-actions button');if(b){const o=b.textContent;b.textContent='✓ 已复制';setTimeout(function(){b.textContent=o;},1500);}});
}
...
function reset(){stopAuto();current=-1;document.getElementById('codePanel').scrollTop=0;render();}
...
function auto(){
  ...
  timer=setInterval(function(){if(current>=STEPS.length-1){stopAuto();return;}next();},speed);
}
function stepOver(){
  ...
  for(let i=current+1;i<STEPS.length;i++){
    const s=STEPS[i];
    if(s.func===curFunc && s.event==='line' && s.line!==curLine){current=i;render();return;}
    if(s.func===curFunc && s.event==='return'){
      let hasMoreLines=false;
      for(let j=i+1;j<STEPS.length;j++){if(STEPS[j].func===curFunc){hasMoreLines=(STEPS[j].event==='line'||STEPS[j].event==='call');break;}}
...
function skipLoop(){
  ...
  for(let i=current+1;i<STEPS.length;i++){
    ...
    if(s.line>maxLine){if(inLoop){current=i;render();return;}maxLine=s.line;}
  }
}
function stepOut(){
  if(current<0||current>=STEPS.length-1)return;
  ...
  for(let i=current+1;i<STEPS.length;i++){
    ...
    for(let j=i+1;j<STEPS.length;j++){if(STEPS[j].func===curFunc){isSuspension=(STEPS[j].event==='call');break;}}
...
document.addEventListener('keydown',function(e){
  ...
  if(e.key==='L'){e.preventDefault();stepOut();}
